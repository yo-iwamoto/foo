...
export const Modal: React.VFC<Props> = ({ modal, close }) => {
  ...
  const ModalPrimaryButton: React.VFC = () => {
    if (modal.link) {
      ...
      return <LinkButton primary text="ログイン" className="w-2/5 h-12" onClick={close} />;
    }
  };
  ...
  return (
    <div className="absolute w-full h-full text-gray-700">
      ...
      <div className={cn({
        ['fixed w-4/5 sm:w-2/3 lg:w-1/3 min-h-1/2 sm:min-w-1/2 z-40 bg-white rounded-lg top-10 sm:top-1/6 left-1/10 sm:left-1/6 lg:left-1/3 opacity-100 p-4 sm:p-6 md:p-8 transform transition-transform']: true,
