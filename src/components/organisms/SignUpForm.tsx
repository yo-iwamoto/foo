...
export const SignUpForm: React.VFC<Props> = ({ firebaseAuth }) => {
  ...
  return (
    <form onSubmit={submitHandler} className="w-4/5 sm:w-3/5 md:w-2/5 mx-auto">
      <Spacer h={12} />
      <ColumnFlexContainer>
        <TextField type="text" value={name} placeholder="ニックネーム" onChange={changeHandler} />
        <Spacer h={6} />
        ...
        <Spacer h={6} />
        <TextField type="password" value={passwordConfirmation} placeholder="パスワード（再確認）" onChange={changeHandler} />
        <Spacer h={6} />
